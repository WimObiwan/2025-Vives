...
}))
{
    ...
    foreach (var group in groupedBySensor)
    {
        ...
        foreach (var record in sRecords)
        {
            ...
            if (avgAnom && ((record.Distance < (Q1 - 1.5 * IQR)) || (record.Distance > (Q3 + 1.5 * IQR))))
...
// Write updated CSV including anomaly column
...
Console.WriteLine("CSV file updated with confirmed anomalies.");
